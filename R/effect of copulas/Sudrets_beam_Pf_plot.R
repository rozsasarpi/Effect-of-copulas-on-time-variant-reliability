...
Pf_df       = readWorksheet(wb, sheet = sheet_name, region = "B50:G59", useCachedValues = TRUE)
...
basic_plot = function(df) {
  ...
  # p = p + ylab(bquote(P[f])) + xlab("t [year]")
  p = p + ylab(bquote(italic(P)[f])) + xlab(expression(paste(italic(t), " [year]")))
...
if (log_plot == 1){
  p1 = p1 + scale_y_log10(breaks = c(1e-5, 1e-3, 1e-1), labels = trans_format("log10", math_format(10^.x)))#,
...
p2 = p2 + ylab(TeX("$\\mathit{P}_{\\mathrm{f}} / \\mathit{P}_{\\mathrm{f,Gauss-DI}}$"))
...
if (save_plot == 1){
  setEPS()
  if (log_plot == 1){
  fig_name = "Sudrets_beam_log_Pf.pdf"
  } else {
    fig_name = "Sudrets_beam_Pf.pdf"
  }
  # 
