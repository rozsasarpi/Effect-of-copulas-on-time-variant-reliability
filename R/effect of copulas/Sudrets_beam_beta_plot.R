...
beta_df     = readWorksheet(wb, sheet = sheet_name, region = "B104:G113", useCachedValues = TRUE)
...
basic_plot = function(df) {
  ...
  p = p + ylab(bquote(beta)) + xlab(expression(paste(italic(t), " [year]")))
...
p2 = p2 + ylab(expression(paste(beta, "/", beta[Gauss-DI])))
...
################################################################################
if (save_plot == 1){
  fig_name = "Sudrets_beam_beta.pdf" 
  ...
  # cowplot::ggsave(filename = file.path(fig_dir, fig_name),
  #            width = 130/25.4, height = 70/25.4,units = "in")
