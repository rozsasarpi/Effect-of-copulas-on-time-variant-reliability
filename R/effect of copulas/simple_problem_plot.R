...
copulas     = c("Gauss-DI", "t (dof=2)-DI", "Gumbel-DI", "rotClayton-180°-DI", "rotGumbel-180°-DI")
...
beta_df$copula = as.factor(copulas)
beta_df$id     = as.factor("raw")
tmp_df$copula  = as.factor(copulas)
...
basic_plot = function(df, log_y = F) {
  p = ggplot(df, aes(x = P_f0, y = beta, color = copula))
  p = p + geom_path()
  ...
  p = p + ylab(bquote(beta)) + xlab(bquote(italic(P)[f0]))
