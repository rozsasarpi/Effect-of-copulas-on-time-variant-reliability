...
# convert ID-s to factors and change the order of specimens for plot - different than the order in excel
...
basic_plot = function(df, log_y = F) {
  ...
  if (log_y == T){
    p = p + scale_y_log10(breaks = c(1e-5, 1e-3, 1e-1), labels = trans_format("log10", math_format(10^.x)))
  }
  ...
  # @@@@@@@@@@@@@@@@@@@@@
  p = p + ylab(bquote(italic(P)[f])) + xlab(TeX("$\\mathit{P}_{\\mathrm{f}}(0)$"))
  # @@@@@@@@@@@@@@@@@@@@@
...
# @@@@@@@@@@@@@@@@@@@@@
p2 = p2 + ylab(TeX("$\\mathit{P}_{\\mathrm{f,Gauss-DI}} / \\mathit{P}_{\\mathrm{f}}$"))
# @@@@@@@@@@@@@@@@@@@@@
